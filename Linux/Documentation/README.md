...
- `docker info`, `docker ps -a`, `docker images`, `docker network ls`, and `docker volume ls` were
  captured with `sudo`.
- Non-root Docker API access is denied by design; Docker administration remains sudo-only for now.
- Use `../Docker/README.md` for the Docker install runbook, validation checklist, troubleshooting
  path, rollback, and interview relevance.
...
- Use `../Portainer/README.md` for the deployment runbook, validation checklist, rollback, and
  security notes.
...
- Use `../RaspberryPi/README.md` and `../../Documentation/Server_Documentation/INFRA01/README.md`
  for evidence.
...
- LINUX01 SSH, Docker, containerd, Portainer container, Portainer volume/network, and Portainer port
  `9443` were validated.
